...
const saveLogInfo  = require("../middlewares/logger/logInfo.js")
...
const getUser = async (req, res, next) => {
    try {
        ...
        user.posts = posts.map((post) => ({
            ...post,
            isMember: post.community?.members.map((member) => member.toString()).includes(user._id.toString()),
 ...
 *
 * @description If the email domain of the user's email is "wenet.com", the user will be
...
const addUser = async (req, res, next) => {
    ...
    try {
        ...
        const fileUrl = req.files?.[0]?.filename ? `${req.protocol}://${req.get('host')}/assets/userAvatars/${req.files[0].filename}` : defaultAvatar;
        const emailDomain = req.body.email.split('@')[1];
        const role = emailDomain == "wenet.com" ? "moderator" : "general";
        ...
        if (isConsentGiven === false) {
            res.status(201).json({
                success: true,
                message: "User created successfully, but consent not given",
                user: {
                    id: newUser._id,
                    name: newUser.name,
                    email: newUser.email,
...
const getModProfile = async(req, res)=>{
    try{
        const moderatorInfo = await User.findById(req.userId).select("-password").lean();
        if(!moderatorInfo){
        ...
        console.log("Error in getModProfile", error.message);
        return res.status(500).json({
            message:"Internal server error"
        })
    }
